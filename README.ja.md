...
クライアントだけ欲しいときは使うバインディングの product を import する（両方入れて Agent Card から選択することも可能）。サーバを実装するときは `A2AServer` に `AgentExecutor` を実装し、`DefaultRequestHandler` に渡す。HTTP を介さず同一プロセスで動かすなら `A2AInProcess` の `A2AClient.inProcess(handler:)` を使う（リモートに切り替えたくなったら transport を差し替えるだけ）。
...
ビルダーで複数パートのメッセージも組み立てられる（文字列リテラルは自動でテキストパート）:
...
do {
    ...
    print(error.reason ?? "unknown")  // TASK_NOT_FOUND（google.rpc.ErrorInfo より。reason は String?）
